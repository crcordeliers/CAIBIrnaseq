...
Some datasets come with gene metadata already attached. If gene metadata is missing, we can fetch it Ensembl's BioMart. This requires a live network connection; if BioMart is unreachable or incompatible with your `biomaRt` version, this chunk may fail to run.
